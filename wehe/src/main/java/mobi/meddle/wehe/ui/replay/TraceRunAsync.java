...
/**
 * AsyncTask to perform the test running functionality.
 * This class was extracted from ReplayActivity for better OOD.
 * Note: AsyncTask was deprecated in Android API 30, but this refactoring
 * keeps it since a complete rewrite would be outside the scope.
 */
public class TraceRunAsync extends AsyncTask<String, String, Void> {
    private final WeakReference<ReplayActivity> activityRef;
    ...
    /**
     * Constructor that takes reference to the activity.
     *
     * @param activity The ReplayActivity that's using this AsyncTask
     */
    public TraceRunAsync(ReplayActivity activity) {
    ...
    protected void onProgressUpdate(@NonNull String... values) {
        ReplayActivity activity = activityRef.get();
        if (activity == null) {
        ...
        if (values[0].equalsIgnoreCase("updateStatus")) {
            ...
            if (values[1].equals(app.getName())) { //make status is being applied to correct app
                ...
                for (ApplicationBean a : activity.getSelectedApps()) {
            ...
            //update progress bar
            ProgressBar prgBar = activity.getProgressBar();
                    ...
                    .setPositiveButton(activity.getString(android.R.string.ok),
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ReplayActivity innerActivity = activityRef.get();
                                    if (innerActivity != null) {
                                        if (exitReplays) {
                                            innerActivity.replayStop();
                                        }
                                        //All tests just finished
                                        if (innerActivity.getDiffApps().size() != 0 ||
                                                innerActivity.getInconclusiveApps().size() != 0) {
                                            innerActivity.displayRerunButtons();
                                        }
                                    }
                                }
                            }).show();
            if (exitReplays) {
                Objects.requireNonNull(activity.getSupportActionBar()).setTitle(R.string.simple_error);
            } else {
                Objects.requireNonNull(activity.getSupportActionBar()).setTitle(R.string.test_results);
            }
        } else {
    ...
    protected void onPreExecute() {
        ...
        ReplayActivity activity = activityRef.get();
        if (activity != null) {
            activity.setReplayOngoing(true);
        }
    }
    ...
    protected void onPostExecute(Void none) {
        ReplayActivity activity = activityRef.get();
        if (activity != null) {
            activity.setReplayOngoing(false);
        }
    }
    ...
    protected Void doInBackground(String... args) {
        ReplayActivity activity = activityRef.get();
        if (activity == null) {
            return null; // Activity no longer exists
        }

        Context context = activity.getApplicationContext();

        //set each app's status to "Waiting"
        for (ApplicationBean app : activity.getSelectedApps()) {
            app.setStatus(activity.getResources().getString(R.string.pending));
        }
        ...
        if (activity.isNetworkUnavailable()) {
            activity.displayNoNetworkDialogue();
            return null;
        }
        ...
        if (activity.isTomography()) { //need to use MLab servers for tomography tests
            serverDisplay = Consts.DEFAULT_SERVER;
        } else {
            serverDisplay = sharedPrefs.getString(activity.getString(R.string.pref_server_key),
        ...
        // to get historyCount
        settings = activity.getSharedPreferences(ReplayActivity.STATUS, Context.MODE_PRIVATE);
        ...
        Config.set("timing", activity.getRunPortTests() ? "false" : "true");
        ...
        if (publicIP.equals("-1")) {
            publishProgress("makeDialog", activity.getString(R.string.simple_error),
                    activity.getString(R.string.error_no_connection), "true");
            return null;
        }
        ...
        for (ApplicationBean app : activity.getSelectedApps()) {
            ...
            if (!activity.isTomography() && rerun) {
        ...
        publishProgress("makeDialog", activity.getString(R.string.replay_finished_title),
                "", "false");
    ...
    private boolean setupServersAndCertificates(@NonNull String server, String metadataServer) {
        ...
        if (server.equals("wehe4.meddle.mobi")) {
            ...
            if (servers.get(servers.size() - 1).equals("")) {
                publishProgress("makeDialog", activity.getString(R.string.simple_error),
                        activity.getString(R.string.error_unknown_host), "true");
                return false;
            }
        }
        ...
        int numTests = activity.isTomography() ? Consts.NUM_TOMOGRAPHY_TESTS : 1;
        ...
        if (metadataServer != null) {
            ...
            if (this.metadataServer.equals("")) { //get IP and certificates for metadata server
                publishProgress("makeDialog", activity.getString(R.string.simple_error),
                        activity.getString(R.string.error_unknown_meta_host), "true");
                return false;
            }
    ...
    private void generateServerCertificate(boolean main) {
        ...
        try {
            ...
            Certificate ca;
            try (InputStream caInput = activity.getResources().openRawResource(main ? R.raw.main : R.raw.metadata)) {
    ...
    private void setInconclusive(String msg) {
        ReplayActivity activity = activityRef.get();
        if (!activity.getInconclusiveApps().contains(app)) {
            activity.addToInconclusiveApps(app);
        }
        app.setError(msg);
        publishProgress("updateStatus", app.getName(), activity.getString(R.string.inconclusive));
    }
    ...
    private boolean runTest(boolean isConfirmation) {
        ...
        for (String channel : types) {
            ...
            if (activity.isNetworkUnavailable()) { //no network available
                activity.displayNoNetworkDialogue();
                return false;
            }
            ...
            if (channel.equalsIgnoreCase("open")) {
                this.appData = unpickleJSON(app.getDataFile(), activity.getActivityContext());
            } else if (channel.equalsIgnoreCase("random")) {
                this.appData = unpickleJSON(app.getRandomDataFile(), activity.getActivityContext());
            } else {
            ...
            try {
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getResources().getString(R.string.create_side_channel));
                ...
                if (randomID == null) {
                    ...
                    setInconclusive(activity.getString(R.string.error_no_user_id));
                    return false;
                }
                 ...
                 */
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getString(R.string.ask4permission));
                    ...
                    if (status.equals("0")) {
                        ...
                        switch (permissionError) {
                            case "1": //server cannot identify replay
                                customError = activity.getString(R.string.error_unknown_replay);
                                break;
                            case "2": //only one replay can run at a time per IP
                                customError = activity.getString(R.string.error_IP_connected);
                                break;
                            case "3": //server CPU > 95%, disk > 95%, or bandwidth > 2000 Mbps
                                customError = activity.getString(R.string.error_low_resources);
                                break;
                            default:
                                customError = activity.getString(R.string.error_unknown);
                        }
                 ...
                 */
                for (CombinedSideChannel sc : sideChannels) {
                    sc.sendMobileStats(Config.get("sendMobileStats"), activity.getApplicationContext());
                }
                 ...
                 */
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getString(R.string.receive_server_port_mapping));
                 ...
                 */
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getString(R.string.create_tcp_client));
                ...
                for (CombinedSideChannel sc : sideChannels) {
                    ...
                    for (String csp : appData.getTcpCSPs()) {
                        ...
                        try {
                            ...
                            setInconclusive(activity.getString(R.string.error_no_connection));
                            return false;
                        }
                ...
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getString(R.string.create_udp_client));
                 ...
                 */
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getString(R.string.run_notf));
                 ...
                 */
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getString(R.string.run_receiver));
                ...
                for (CombinedSideChannel sc : sideChannels) {
                    CombinedAnalyzerTask analyzerTask = new CombinedAnalyzerTask(app.getTime() / 2.0,
                            appData.isTCP(), numOfTimeSlices.get(sc.getId()), activity.getRunPortTests()); //throughput logger
                 ...
                 */
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getString(R.string.run_sender));
                ...
                CombinedQueue queue = new CombinedQueue(appData.getQ(), jitterBeans, analyzerTasks,
                        activity.getRunPortTests() ? Consts.REPLAY_PORT_TIMEOUT : Consts.REPLAY_APP_TIMEOUT);
                 ...
                 */
                publishProgress("updateStatus", app.getName(), iteration + "/"
                        + types.length + " " + activity.getString(R.string.send_done));
                ...
                if (activity.getRunPortTests() && channel.equalsIgnoreCase("random")) {
        ...
        try {
            ...
            if (!portBlocked) { //skip Step 1 and step 2 if port blocked
                ...
                if (analysisResults.size() != analyzerServerUrls.size()) {
                    setInconclusive(activity.getString(R.string.error_analysis_fail));
                    return false;
                }
                ...
                publishProgress("updateStatus", app.getName(), activity.getString(R.string.waiting));
                ...
                for (String url : analyzerServerUrls) {
                    for (int i = 0; ; i++) { //3 attempts to get analysis from sever
                        ...
                        if (i < 3) { //wait 2 seconds to try again
                            ...
                            if (activity.getRunPortTests()) { //"the port 80 issue"
                                portBlocked = true;
                                Log.i("Result Channel", "Can't retrieve result, port blocked");
                                break;
                            } else {
                                setInconclusive(activity.getString(R.string.not_all_tcp_sent_text));
                                return false;
                            }
                        }
                    }
            ...
            if (activity.isTomography()) {
                ...
                if (random.nextInt(2) == 1) {
                    differentiationNetwork = activity.getCarrier();
                }
            }
            ...
            if ((!userID.trim().equalsIgnoreCase(randomID))
                ...
                setInconclusive(activity.getString(R.string.error_result));
                return false;
            }
...
//                inconclusive = true;
            ...
            if ((inconclusive || differentiation) && confirmationReplays && !isConfirmation && !activity.isTomography()) {
                publishProgress("updateStatus", app.getName(), activity.getString(R.string.confirmation_replay));
            ...
            if (activity.isTomography()) {
                ...
                displayStatus = differentiationNetwork.equals("") ? activity.getString(R.string.tomo_failed)
                        : activity.getString(R.string.tomo_succ);
            } else if (inconclusive) {
                saveStatus = "inconclusive";
                displayStatus = activity.getString(R.string.inconclusive);
                activity.addToInconclusiveApps(app);
            } else if (differentiation) {
                saveStatus = "has diff";
                displayStatus = activity.getString(R.string.has_diff);

                String error = activity.getRunPortTests() ? activity.getString(R.string.test_blocked_port_text) :
                        activity.getString(R.string.test_blocked_app_text);
                if (!portBlocked) {
                    if (xputOriginal > xputTest) {
                        error = activity.getRunPortTests() ? activity.getString(R.string.test_prioritized_port_text) :
                                activity.getString(R.string.test_prioritized_app_text);
                    } else {
                        error = activity.getRunPortTests() ? activity.getString(R.string.test_throttled_port_text) :
                                activity.getString(R.string.test_throttled_app_text);
                    }
                }
                app.setError(error);
                ...
                activity.addToDiffApps(app);
            } else {
                saveStatus = "no diff";
                displayStatus = activity.getString(R.string.no_diff);
            }
            ...
            response.put("isPort", activity.getRunPortTests());
            ...
            response.put("carrier", activity.getCarrier());
            if (activity.isTomography()) {
